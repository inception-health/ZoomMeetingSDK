...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Listening in", @"") style: UIBarButtonItemStylePlain target: self action: @selector(onListeningInBtnClicked:)];
...
- (void)onListeningInBtnClicked:(id)sender
{
    AppDelegate *appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Listening in"
                                                                             message:nil
                                                                      preferredStyle:UIAlertControllerStyleActionSheet];
    MobileRTCMeetingService *ms = [[MobileRTC sharedRTC] getMeetingService];
    if (!ms) return;
    
    NSArray * listenLanList = [ms getInterpreterAvailableLanguages];
    for (MobileRTCInterpretationLanguage *lan in listenLanList) {
        [alertController addAction:[UIAlertAction actionWithTitle:lan.getLanguageName
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *action) {
                                                                BOOL ret = [ms setInterpreterListenLan:[lan getLanguageID]];
                                                                NSLog(@"setInterpreterListenLan====>%@", @(ret));
                                                          }]];
    }
