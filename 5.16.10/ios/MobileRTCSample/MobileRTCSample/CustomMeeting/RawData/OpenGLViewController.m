...
//        _localShareVC.view.frame = CGRectMake(0, 0, 500, 500);
    }
...
- (ControlBar *)controlBarView {
    if (!_controlBarView) {
        ...
        __weak typeof(self) weakSeft = self;
        _controlBarView.MyBlock = ^(int tag) {
            NSLog(@"MyBlock -- %d",tag);
            if (tag == kTagButtonShare) {
                MobileRTCMeetingService *ms = [[MobileRTC sharedRTC] getMeetingService];
                
                if(ms.isStartingShare) {
                    [ms stopAppShare];
                    [weakSeft.localShareVC removeFromParentViewController];
                    [weakSeft.localShareVC.view removeFromSuperview];
                    if (weakSeft.fullRender.videoType == MobileRTCVideoType_ShareData) {
                        [weakSeft noVideofailBack];
                    }
                }
                else {
                    [weakSeft addChildViewController:weakSeft.localShareVC];
                    [weakSeft.view addSubview:weakSeft.localShareVC.view];
                    [weakSeft.localShareVC didMoveToParentViewController:weakSeft];
                    [weakSeft.view insertSubview:weakSeft.localShareVC.view belowSubview:weakSeft.topBar];
                    [[[MobileRTC sharedRTC] getMeetingService] appShareWithView:weakSeft.localShareVC.view];
                }
...
- (void)onSinkSharingStatus:(MobileRTCSharingStatus)status userID:(NSUInteger)userID {
    switch (status) {
        case MobileRTCSharingStatus_Self_Send_Begin:
        {
            ...
            MobileRTCRenderer *renderer = [[MobileRTCRenderer alloc] initWithDelegate:self];
            [renderer subscribe:myUserID videoType:MobileRTCVideoType_ShareData];
            [self.renderArr addObject:renderer];
            OpenglView *view = [[OpenglView alloc] initWithFrame:CGRectZero];
            ViewItem *item = [[ViewItem alloc] init];
        ...
        case MobileRTCSharingStatus_Self_Send_End:
        {
            NSInteger myUserID = [[[MobileRTC sharedRTC] getMeetingService] myselfUserID];
            for(ViewItem *item in self.bottomView.viewArray) {
                if (item.userId == myUserID && item.renderer.videoType == MobileRTCVideoType_ShareData) {
                    [item.renderer unSubscribe];
                    [self.bottomView removeThumberViewItem:item];
                }
            }
        }
            break;
...
//    }
    ...
    for (ViewItem *item in items) {
        ...
        if (renderer.videoType == MobileRTCVideoType_ShareData) {
            NSLog(@"sub share data ..");
        }
...
- (void)onShareAudioRawDataReceived:(MobileRTCAudioRawData *)rawData
{
    NSLog(@"onMobileRTCMixedAudioRawData");
}
...
- (void)pinThumberViewItem:(ViewItem *)item {
    ...
    if ([self.view.subviews containsObject:self.localShareVC.view]) {
        [self.localShareVC.view removeFromSuperview];
    }
    if (item.renderer.videoType == MobileRTCVideoType_VideoData) {
        NSLog(@"Full render subcribe VideoData");
        [self.fullRender subscribe:item.userId videoType:MobileRTCVideoType_VideoData];
    }
    else {
        NSLog(@"Full render subcribe ShareData");
        [self.fullRender subscribe:item.userId videoType:MobileRTCVideoType_ShareData];
    }
