...
{ 
    ...
    char *newYBuffer = NULL;
    if (rawData.alphaBufferLen > 0 && rawData.alphaBuffer != NULL && bufferLen == rawData.alphaBufferLen) {
        newYBuffer = (char *)malloc(bufferLen);
        int alphaBufferLen = rawData.alphaBufferLen;
        for (int idx = 0; idx < bufferLen; idx++) {
            if (idx < alphaBufferLen) {
                int val = rawData.alphaBuffer[idx];
                if ((val & 1) == 1) {
                    newYBuffer[idx] = rawData.yBuffer[idx];
                } else {
                    newYBuffer[idx] = 0;
                }
            } else {
    ...
    glBindTexture(GL_TEXTURE_2D, _textureYUV[TEXY]);
    if (newYBuffer != NULL) {
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, (GLsizei)w, (GLsizei)h, GL_RED_EXT, GL_UNSIGNED_BYTE, newYBuffer);
    } else {
