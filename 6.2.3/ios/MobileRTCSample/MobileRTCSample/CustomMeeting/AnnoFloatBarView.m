...
- (void)showActionView {
    ...
    UIColor *toolColor = [annoService getToolColor];
...
- (void)onHighlightButtonClicked:(id)sender
{
    ...
    if (annoService)
    {
        [annoService setToolType:MobileRTCAnnoTool_HighLighter];
    }
}
...
- (void)onColorButtonClicked:(id)sendor {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        UIColor *color = [annoService getToolColor];
        UIImage *back = [self imageWithColor:[UIColor clearColor] size:CGSizeMake(20, 20) andRoundSize:0];
...
{
    MobileRTCAnnotationService *annoService = [[MobileRTC sharedRTC] getAnnotationService];
    if (!annoService) return;
    
    MobileRTCAnnoTool type = [annoService getToolType];
    if (MobileRTCAnnoTool_AutoStampQm == type) type = MobileRTCAnnoTool_None;
    [annoService setToolType:type+1];
}
...
- (void)onWidthButtonClicked:(id)sender {
    MobileRTCAnnotationService *annoService = [[MobileRTC sharedRTC] getAnnotationService];
    if (annoService) {
        NSUInteger width = [annoService getToolWidth];
        width += 8;
        if (width > 45) width = 1;
        [annoService setToolWidth:width];
    }
}
...
- (void)showCleanTestMenu:(id)sender
{
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"Clear my"
        ...
        MobileRTCAnnotationService *annoService = [[MobileRTC sharedRTC] getAnnotationService];
        if (annoService) [annoService clear:MobileRTCAnnoClearType_My];
                                                        }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"Clear all"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action) {
        MobileRTCAnnotationService *annoService = [[MobileRTC sharedRTC] getAnnotationService];
        if (annoService) [annoService clear:MobileRTCAnnoClearType_All];
                                                      }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"Clear others"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action){
        MobileRTCAnnotationService *annoService = [[MobileRTC sharedRTC] getAnnotationService];
        if (annoService) [annoService clear:MobileRTCAnnoClearType_Others];
    }]];
