...
#define CameraControl_Range  50.f

@interface ThumbView ()<UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate, MobileRTCCameraControlDelegate>
...
-(void)handleLongPress:(UILongPressGestureRecognizer *)gestureRecognizer
{
    if(gestureRecognizer.state == UIGestureRecognizerStateBegan)
    {
        ...
        MobileRTCCameraControlService *cameraControl = [[MobileRTC sharedRTC] getCameraControlService:userID];
        cameraControl.delegate = self;
        BOOL canControlCamera = [cameraControl canControlCamera];
        
        [alertController addAction:[UIAlertAction actionWithTitle:[NSString stringWithFormat:@"camera control: canControlCamera=%@",@(canControlCamera)]
                                                          style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction *action) {
            MobileRTCSDKError ret = [cameraControl requestControlRemoteCamera];
            NSLog(@"cameraControl: canControlCamera ===>%@", @(canControlCamera));
                                                        }]];
        if (canControlCamera) {
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:requestControlRemoteCamera"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCSDKError ret = [cameraControl requestControlRemoteCamera];
                NSLog(@"cameraControl: requestControlRemoteCamera ===>%@", @(ret));
                                                            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:giveUpControlRemoteCamera"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCSDKError ret = [cameraControl giveUpControlRemoteCamera];
                NSLog(@"camera control:giveUpControlRemoteCamera ===>%@", @(ret));
                                                            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:turnLeft"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCCameraControlService *cameraControl = [[MobileRTC sharedRTC] getCameraControlService:userID];
                MobileRTCSDKError ret = [cameraControl turnLeft:CameraControl_Range];
                NSLog(@"cameraControl: turnLeft ===>%@", @(ret));
                                                            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:turnRight"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCCameraControlService *cameraControl = [[MobileRTC sharedRTC] getCameraControlService:userID];
                MobileRTCSDKError ret = [cameraControl turnRight:CameraControl_Range];
                NSLog(@"cameraControl: turnRight ===>%@", @(ret));
                                                            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:turnUp"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCCameraControlService *cameraControl = [[MobileRTC sharedRTC] getCameraControlService:userID];
                MobileRTCSDKError ret = [cameraControl turnUp:CameraControl_Range];
                NSLog(@"cameraControl: turnUp ===>%@", @(ret));
                                                            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:turnDown"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCCameraControlService *cameraControl = [[MobileRTC sharedRTC] getCameraControlService:userID];
                MobileRTCSDKError ret = [cameraControl turnDown:CameraControl_Range];
                NSLog(@"cameraControl: turnDown ===>%@", @(ret));
                                                            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:zoomIn"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCCameraControlService *cameraControl = [[MobileRTC sharedRTC] getCameraControlService:userID];
                MobileRTCSDKError ret = [cameraControl zoomIn:CameraControl_Range];
                NSLog(@"cameraControl: zoomIn ===>%@", @(ret));
                                                            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"camera control:zoomOut"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction *action) {
                MobileRTCCameraControlService *cameraControl = [[MobileRTC sharedRTC] getCameraControlService:userID];
                MobileRTCSDKError ret = [cameraControl zoomOut:CameraControl_Range];
                NSLog(@"cameraControl: zoomOut ===>%@", @(ret));
                                                            }]];
        }
...
{
    NSLog(@"cameraControl:onCameraControlRequestResult userId=%ld isApproved=%@", userId, @(isApproved));
}
