...
@property (nonatomic, strong) MobileRTCAudioSourceHelper *audioSourceHelper;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if ([[MobileRTC sharedRTC] hasRawDataLicense]) {
            ...
            self.audioSourceHelper = [[MobileRTCAudioSourceHelper alloc] init];
        }
...
- (void)dealloc {
    ...
    self.shareSourceHelper = nil;
    self.audioSourceHelper = nil;
}
...
- (void)onMoreButtonClicked:(id)sender
{
    ...
    if ([ms isSupportWhiteBoard]) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"showDashboardView"
          style:UIAlertActionStyleDefault
        handler:^(UIAlertAction *action) {
            AppDelegate *appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
            [ms showDashboardView];
    ...
    if ([ms canDoPolling]) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"Polling"
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *action) {
                                                              AppDelegate *appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;

                                                              PollingViewController *VC = [[PollingViewController alloc] init];
                                                              UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:VC];
                                                              nav.modalPresentationStyle = UIModalPresentationFullScreen;
                                                              [[appDelegate topViewController] presentViewController:nav animated:YES completion:NULL];
                                                          }]];
    ...
    if (ms)
    {
        ...
        if (hasLicense) {
            ...
            [alertController addAction:[UIAlertAction actionWithTitle:@"Send Rawdata Share - YUV Data"
                ...
                [self.shareSourceHelper setExternalShareSource:self.yuvShareAdapter andAudioSource:nil];
            }]];
            ...
            [alertController addAction:[UIAlertAction actionWithTitle:@"Stop Rawdata Share - YUV Data"
                ...
                [self.shareSourceHelper setExternalShareSource:nil andAudioSource:nil];
                
                MobileRTCMeetingService *ms = [[MobileRTC sharedRTC] getMeetingService];
                if (ms && ms.isStartingShare) [ms stopAppShare];
            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"Start Rawdata Audio"
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *action) {
                self.pcmAudioAdapter = [[SendPCMAdapter alloc] init];
                [self.audioSourceHelper setExternalAudioSource:self.pcmAudioAdapter];
            }]];
            
            [alertController addAction:[UIAlertAction actionWithTitle:@"Stop Rawdata Audio"
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *action) {
                self.pcmAudioAdapter = nil;
                [self.audioSourceHelper setExternalAudioSource:nil];
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"Show AIC Pannel"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action) {
        MobileRTCMeetingService *ms = [[MobileRTC sharedRTC] getMeetingService];
        [ms showDynamicNoticeForAICompanionPanel:self.superview originPoint:CGPointMake(0, 60)];
    }]];
    ...
    if ([ms isEmojiReactionEnabled]) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"Send reaction"
                                                          style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction *action) {
                                                            MobileRTCSDKError ret = [ms sendEmojiReaction:MobileRTCEmojiReactionType_Clap];
                                                            NSLog(@"Send reaction sendEmojiReaction ---> %@", @(ret));
                                                        }]];
        
        [alertController addAction:[UIAlertAction actionWithTitle:@"Send emoji feedback"
                                                          style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction *action) {
                                                            MobileRTCSDKError ret = [ms sendEmojiFeedback:MobileRTCEmojiFeedbackType_Yes];
                                                            NSLog(@"Send feedback sendEmojiFeedback ---> %@", @(ret));
                                                        }]];
        [alertController addAction:[UIAlertAction actionWithTitle:@"cancel emoji feedback"
                                                          style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction *action) {
                                                            MobileRTCSDKError ret = [ms cancelEmojiFeedback];
                                                            NSLog(@"Send feedback cancelEmojiFeedback ---> %@", @(ret));
                                                        }]];
    }
    ...
    if (![ms isChatDisabled]) {
        [alertController addAction:[UIAlertAction actionWithTitle:@"Send a group chat message to all"
            ...
            msg.chatMessageType = MobileRTCChatMessageType_To_All;
        ...
        if ([ms isWebinarMeeting]) {
            [alertController addAction:[UIAlertAction actionWithTitle:@"Send a group chat message to Panelists"
                                                            ...
                                                            handler:^(UIAlertAction *action) {
                MobileRTCMeetingChat *msg= [[MobileRTCMeetingChat alloc]init];
                msg.chatMessageType = MobileRTCChatMessageType_To_All_Panelist;
                msg.content = @"This is a private chat message";
                MobileRTCSendChatError error = [ms sendChatMsg:msg];
                NSLog(@"SendChat:sendChatToGroup ---> %@", @(error));
                                                            }]];
            ...
            if ([ms isMeetingHost] || [ms isMeetingCoHost]) {
                [alertController addAction:[UIAlertAction actionWithTitle:@"Send a group chat message to Waiting Room"
                                                                ...
                                                                handler:^(UIAlertAction *action) {
                    MobileRTCMeetingChat *msg= [[MobileRTCMeetingChat alloc]init];
                    msg.chatMessageType = MobileRTCChatMessageType_To_WaitingRoomUsers;
                    msg.content = @"This is a private chat message";
                    MobileRTCSendChatError error = [ms sendChatMsg:msg];
                    NSLog(@"SendChat:sendChatToGroup ---> %@", @(error));
    ...
    if ([ms isMeetingHost] || [ms isMeetingCoHost]) {
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:muteOnEntryString
                                                          style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction *action) {
                                                            [ms enableMuteOnEntry:![ms isMuteOnEntryOn] allowUnmuteBySelf:[ms isParticipantsUnmuteSelfAllowed]];
                                                        }]];
...
#pragma live transcription demo
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"enable GrabShareWithoutReminder"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action) {
        [[[MobileRTC sharedRTC] getMeetingSettings] enableGrabShareWithoutReminder:YES];
                                                      }]];
    
    [alertController addAction:[UIAlertAction actionWithTitle:@"disable GrabShareWithoutReminder"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action) {
        [[[MobileRTC sharedRTC] getMeetingSettings] enableGrabShareWithoutReminder:NO];
                                                      }]];
    
    [alertController addAction:[UIAlertAction actionWithTitle:@"canStartShare"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action) {
        MobileRTCCannotShareReasonType ret = [ms canStartShare];
        NSLog(@"can share ---> %@", @(ret));
                                                      }]];
    
    [alertController addAction:[UIAlertAction actionWithTitle:@"canStartShareWhiteboard"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action) {
        MobileRTCCannotShareReasonType ret = [ms canStartShareWhiteboard];
        NSLog(@"whitebord can share ---> %@", @(ret));
                                                      }]];
