...
- (void)layoutSubviews {
    ...
    _shareBtn.frame = CGRectMake(0, CGRectGetMaxY(_videoBtn.frame), button_width, button_width * ([UIImage imageNamed:@"icon_meeting_share"].size.height/[UIImage imageNamed:@"icon_meeting_share"].size.width));
    _moreBtn.frame = CGRectMake(0, CGRectGetMaxY(_shareBtn.frame), button_width, button_width * ([UIImage imageNamed:@"icon_video_more"].size.height/[UIImage imageNamed:@"icon_video_more"].size.width));
...
- (void)initSubView {
    ...
    [_shareBtn setImage:[UIImage imageNamed:@"icon_video_share_stop"] forState:UIControlStateSelected];
...
- (void)onBarButtonClicked:(UIButton *)sender
{
    switch (sender.tag) {
        ...
        case kTagButtonMore:
        {
            ...
            [alertController addAction:[UIAlertAction actionWithTitle:@"Send Rawdata Share - YUV with audio"
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *action) {
                self.yuvShareAdapter = [[SendYUVShareAdapter alloc] init];
                self.yuvShareAdapter.sharing = YES;
                [self.shareSourceHelper setExternalShareSource:self.yuvShareAdapter andAudioSource:self.yuvShareAdapter];
            }]];
