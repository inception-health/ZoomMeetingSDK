    ...
    while (![NSThread currentThread].isCancelled) {
        ...
        if (size == 0) {
         ...
         dispatch_sync(dispatch_get_main_queue(), ^{
             [self.shareRawdataSender sendShareFrameBuffer:(char *)yuvFrame width:self.width height:self.height frameLength:self.height*self.width*1.5 format:MobileRTCFrameDataFormat_I420];
             free(yuvFrame);
         });
