    ...
    while (![NSThread currentThread].isCancelled) {
        ...
        if (size == 0) {
         ...
         dispatch_sync(dispatch_get_main_queue(), ^{
            [self.videoRawdataSender sendVideoFrame:(char *)yuvFrame width:self.width height:self.height dataLength:self.height*self.width*1.5 rotation:MobileRTCVideoRawDataRotationNone format:MobileRTCFrameDataFormat_I420];
             free(yuvFrame);
         });
