...
- (TopPanelView *)topPanelView
{
    if (!_topPanelView)
    {
        ...
        _topPanelView.shrinkButtonClickBlock = ^(void) {
            ...
            [strongSelf enableGuestureRecognizer];
            strongSelf.isFullScreenMode = NO;
            strongSelf.oriTransform = strongSelf.view.transform;
            [strongSelf changeToPIPModeNeedAnimate:strongSelf.view];
            [strongSelf initGuestureRecognizer];
        };
    }
...
{
    NSLog(@"onCustomWaitingRoomDataUpdated: %@", data);
}

- (void)onWaitingRoomUserNameChanged:(NSInteger)userID userName:(NSString *)userName {
    NSLog(@"- onWaitingRoomUserNameChanged:userName %d %@",userID,userName );
}
