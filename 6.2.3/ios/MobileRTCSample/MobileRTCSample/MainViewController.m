...
@interface MainViewController ()<UIAlertViewDelegate, UIActionSheetDelegate, MobileRTCMeetingServiceDelegate,MobileRTCMeetingShareActionItemDelegate,MobileRTCDirectShareServiceDelegate,MobileRTCAICompanionSmartSummaryHelperDelegate
...
@property (strong, nonatomic) SDKReminderDemo *reminderControl;
@end
...
//    }];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self doSDKInitAndAuth];
    });
}

- (void)doSDKInitAndAuth {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Choose ZoomUI or CustomUI" message:@"You need choose ZoomUI or CustomUI when MoblieRTC initialize." preferredStyle:UIAlertControllerStyleAlert];

    UIAlertAction *zoomuiAction = [UIAlertAction actionWithTitle:@"ZoomUI" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"You choose ZoomUI!");
        // SDK init
        UINavigationController *vc = (UINavigationController *)[UIApplication sharedApplication].delegate.window.rootViewController;
        [SDKInitPresenter SDKInit:vc isCustomUI:NO];

        // MobileRTC Authorize
        self.authPresenter = [[SDKAuthPresenter alloc] init];
        [self.authPresenter SDKAuth:KjwtToken];
        self.reminderControl = [[SDKReminderDemo alloc] init];
        [self.reminderControl setReminderDelegate];
    }];
    [alertController addAction:zoomuiAction];
    
    UIAlertAction *customUIAction = [UIAlertAction actionWithTitle:@"CustomUI" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"You choose CustomUI!");
        // SDK init
        UINavigationController *vc = (UINavigationController *)[UIApplication sharedApplication].delegate.window.rootViewController;
        [SDKInitPresenter SDKInit:vc isCustomUI:YES];
...
- (void)onMeetNow:(id)sender
{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0)
    {
        ...
        if ([[MobileRTC sharedRTC] isEnabledCustomizeMeetingUI])
        {
